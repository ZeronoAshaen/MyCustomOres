...
private ItemIngot itemIngot;
	...
	public boolean hasCustomLoader(IResourceManager manager, ResourceLocation location)
	{
        ...
        try
        {
        	...
        	LogHelper.info("Detected override for " + itemIngot.getOreName() + " ingot.");
        	return false;
        }
        catch(IOException e)
        {
	...
	public boolean load(IResourceManager manager, ResourceLocation location)
	{
		...
		try
		{
			...
			underImage = ImageIO.read(iResourceShape.getInputStream());
			...
			for(int i=0; i<alphaImage.getHeight(); i++)
			{
				...
				{
					int alphaLevel = ColorHelpers.getAlpha(underImage, j, i);
					if(alphaLevel > 0)
					{
						g.drawLine(j, i, j, i);
					}
